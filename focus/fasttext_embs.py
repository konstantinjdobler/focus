...
CACHE_DIR = (Path(os.getenv("XDG_CACHE_HOME", "~/.cache")) / "focus").expanduser().resolve()
...
def train_fasttext(
    ...
    cache_file = (
        CACHE_DIR / "data" / f"{data_file_name}_tokenized_{target_tokenizer_hash}{data_file_suffix}"
    )
    ...
    if cache_file.exists():
        ...
        dataset = dataset.map(
            lambda sample: {
                "text": " ".join([token for token in target_tokenizer.tokenize(sample["text"])])
            },
            num_proc=processes,
        )
        if cache_tokenized_text:
            ...
            logger.success(
                f"Tokenized target language training data for fasttext written to {cache_file}..."
            )
        else:
    ...
    logger.info(
        f"Training fasttext model on {f'tokenized {text_path}' if cache_tokenized_text else cache_file}..."
    )
...
def load_target_token_embedding(
    *,
    ...
    fasttext_epochs: int = None,
    fasttext_embedding_dim: int = None,
...
def train_or_load_fasttext_model(
    ...
    if not model_cache_path.exists():
        fasttext_model = train_fasttext(
            text_path, target_tokenizer, epochs=epochs, dim=dim, processes=processes
        )
